...
/**
 * AttributMetaData h�lt alle Beschreibungsdaten �ber die Attribute.<br>
 * <br>
...
public class AttributMetaData {
   ...
   * Die MataDaten m�ssen nur einmal im System vorhanden sein.
   */
  private static AttributMetaData metaData = null;
  
  /**
   * Diese Methode gibt abh�ngig von den Parametern die Anforderung-ID
   * zur�ck.
   * 
   * @param bundleName
  ...
  /**
   * Diese Methode gibt abh�ngig von den Parametern den Attribut-Label
   * Text zur�ck. Je nach Sprache_code in der entsprechenden Sprache.
   * Ist der Sprach_code = ? wird der technische Name =-> die attributID
   * zusammen mit der Anforderungs-Id zur�ck gegeben.
   * 
   * @param bundleName
   *          "ch.kausoft.attribut.person.messages"
   * @param attributID
   ...
   * Es gen�gt wenn die AttributMetaData pro Applikation nur einmal
   * vorhanden sind. Dieses Singelton ist nicht gesichert. Das heist es
   * gibt keine Referenzen die daf�r sorgen dass dieses Singelton vom
   * Garbage Collector nicht etwa gel�scht w�rde. Dies schadet
   * allerdings nicht, weil es nur transiente Daten enth�lt.
   * 
  ...
  /**
   * Der vorhanden Buffer wird gel�scht, das f�hrt dazu dass bei Bedarf
